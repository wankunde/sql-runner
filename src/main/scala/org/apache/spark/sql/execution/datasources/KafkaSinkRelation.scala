 ...
 * @date 2020-07-13.
 */
case class KafkaSinkRelation(override val schema: StructType,
  ...
  def parseAvroSchemaByDF(): Schema = {
    ...
    val avroFields = schema.fields.map { f =>
  ...
  def doSend[T: ClassTag](value: T)(implicit producer: KafkaProducer[String, T]): Unit = {
    producer.send(new ProducerRecord(topic, value))
  }

  override def insert(data: DataFrame, overwrite: Boolean): Unit = {
    val upsertCount = sparkSession.sparkContext.longAccumulator(s"kafka_${topic}")
    data.rdd.foreachPartition { iterator =>
      implicit val stringProducer = initProducer[String]()
      implicit val avroProducer = initProducer[Record]()

      val rateLimiter = RateLimiter.create(maxRatePerPartition)
      log.info(s"maxRatePerPartition kafka sink: $maxRatePerPartition rate limiter $rateLimiter")
    ...
    reportMetrics(s"stats.counters.insight.sink.kafka.${topic}.count", upsertCount.value)
    logInfo(s"写入kafka($topic) 记录条数: ${upsertCount.value}")
  }
}
